...
config_epg_cache_external_offset = config.get("epg_cache_external_offset", -1)
...
for channel in config_epg_internal:
    ...
    if from_raw_channel is None:
        print(f"from channel {tvg_from_id} not found, skip")
        continue
    ...
    if to_raw_channel is None:
        print(f"to channel {tvg_to_id} not found, skip")
        continue
    ...
    print(f"add internal epg mapping: from channel {tvg_from_id} to channel {tvg_to_id}")
    # 清理目标频道已有的 EPG 数据
    if tvg_to_id in channel_with_epg_list:
        programme_list = [p for p in programme_list if p["channel"] != str(tvg_to_id)]
        print(f"channel {tvg_to_id} in channel_with_epg_list, clear its programme in programme_list for rewrite")
    # 添加频道到频道列表
...
print(f"Added {len(new_programmes)} new programme entries")
...
for epg in config_epg_external:
    ...
    for channel in epg["channel"]:
        ...
        if raw_channel == None:
            print(f"channel {tvg_id_iptv} not found, skip")
            continue
        if raw_channel.get("channelName") != channel["tvg_name"]:
            print(f"channel {tvg_id_iptv}'s channelName not match {channel['tvg_name']}, skip")
            continue
        ...
        if ext_has_epg:
            ...
            print("add external epg mapping: from external", tvg_id_ext , "to channel", tvg_id_iptv)
            if tvg_id_iptv in channel_with_epg_list:
                ...
                print(f"channel {tvg_id_iptv} in channel_with_epg_list, clear its programme in programme_list for rewrite")
    ...
    for epg_tvg_id_ext in ext_id_dict:
        channel_code = ext_id_dict[epg_tvg_id_ext]
        for i in range(config_epg_offset_start, config_epg_cache_external_offset):
            datestr = datetime_now + datetime.timedelta(days=i)
            datestr = get_time_str(datestr, "%Y%m%d")
    ...
    for programme in ext_epg_data.findall('programme'):
        ...
        if zh_title is not None and time_start is not None and time_stop is not None:
            ...
            programme_list.append({
                "channel": str(ext_id_dict[channel]),
                "start": time_start,
                "stop": time_stop,
                "title": zh_title
            })
            date = time_start[0:8]
            ext_epg_cache_data[ext_id_dict[channel]] = ext_epg_cache_data.get(ext_id_dict[channel], {})
            ext_epg_cache_data[ext_id_dict[channel]][date] = ext_epg_cache_data[ext_id_dict[channel]].get(date, [])
            ext_epg_cache_data[ext_id_dict[channel]][date].append({
                "channel": str(ext_id_dict[channel]),
                "start": time_start,
                "stop": time_stop,
                "title": zh_title
